...
def test_find_optimal_builds_success():
    ...
    result = find_optimal_builds(budget=1400.0, purpose="gaming")
...
def test_find_optimal_builds_brand_preferences():
    ...
    result = find_optimal_builds(
        budget=1500.0, purpose="gaming", cpu_brand="Intel", gpu_brand="NVIDIA"
    )
    ...
    for config in result["configurations"]:
        ...
        assert (
            "GeForce" in config["parts"]["GPU"]["name"]
            or "RTX" in config["parts"]["GPU"]["name"]
        )


def test_find_optimal_builds_too_low_budget():
    """Verify that solver returns 0 configurations when budget is extremely low."""
    result = find_optimal_builds(budget=200.0, purpose="office")
