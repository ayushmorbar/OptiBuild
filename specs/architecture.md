...
|---|---|---|
| **Completeness** (0–1) | Deterministic | Fraction of the required-category set present in `decision_variables`. V1 required set: `cpu, motherboard, memory, internal-hard-drive, power-supply, case, cpu-cooler` (+ `video-card` unless intent implies iGPU). Also: every objective/constraint term resolvable (already hard-guaranteed by §2 validators → contributes 1.0 or reveals a bug). |
...
### CSV metadata schema (`data/pc-csv/metadata.json`)

One entry per CSV; hand-authored for V1 (with `record_count`/types generated by a script).
...
- **Systematic (`clean_systematic`, always)**: drop rows with null/negative/zero `price`;
  coerce declared numeric columns (rows failing coercion dropped, counted); IQR-based extreme
  outlier removal on `price`; category-specific fixed fixes — e.g. split `memory.speed`
  `"5,6000"` into `ddr_gen`/`speed_mhz` and `memory.modules` `"2,16"` into
  `module_count`/`module_gb`.
...
- **Variables**: `x[c,i] ∈ {0,1}` for each row `i` of category `c` (post-pre-filter, capped at
  top-200 rows per category by ascending price to bound model size).
...
│   ├── server.py                     # FastMCP tool registration (§4, 8 tools)
│   ├── catalog.py                    # metadata.json load, search_datasets (+ RAG fallback)
│   ├── store.py                      # dataset_handle registry (session-scoped DataFrames)
│   ├── cleaning.py                   # clean_systematic rules
│   ├── safe_ops.py                   # expr allowlist gate (numexpr), CleanOp executor, effect validation (§8)
│   ├── kb.py                         # knowledge_base.json + resolve_thresholds (node G)
│   ├── prefilter.py                  # pandas pre-filter (node H)
│   ├── cpsat.py                      # model builder: vars, derived-expr compiler, compat rules (§7)
│   └── ranking.py                    # K-candidate enumeration + pymcdm TOPSIS
├── data/
│   ├── pc-csv/                       # 25 category CSVs (V1 dataset)
│   ├── pc-csv/metadata.json          # §6 catalog
│   ├── knowledge_base.json           # use-case thresholds + microarch→socket map (§2b, §7)
│   └── compatibility_rules.json      # declarative compat rule table (§7)
├── eval/
...
├── tests/
│   ├── test_schema.py                # pivot validators, formula grammar, weight normalization
│   ├── test_gates.py                 # Gate 1/2 truth table
│   ├── test_safe_ops.py              # expr allowlist rejections, CleanOp validation, effect guards (security tests)
│   ├── test_cpsat.py                 # compat clauses, budget, exactly-one, INFEASIBLE reporting
│   └── test_topsis.py                # weight application, ranking determinism
