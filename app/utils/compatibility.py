...
def check_socket_compatible(cpu: dict, motherboard: dict) -> bool:
    ...
    return cpu.get("specs", {}).get("socket") == motherboard.get("specs", {}).get(
        "socket"
    )
...
def check_ram_compatible(ram: dict, motherboard: dict) -> bool:
    ...
    return ram.get("specs", {}).get("generation") == motherboard.get("specs", {}).get(
        "ram_generation"
    )
...
def check_build_compatible(
    ...
    return (
        check_socket_compatible(cpu, motherboard)
        and check_ram_compatible(ram, motherboard)
        and check_case_compatible(motherboard, case)
        and check_cooler_socket_compatible(cpu, cooler)
        and check_psu_wattage_compatible(cpu, gpu, psu)
    )
